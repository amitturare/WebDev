...
// Select form, input, and result article
const form = document.querySelector('.lorem-form');
const amount = document.getElementById('amount');
const result = document.querySelector('.lorem-text');

// Configure form for submit
form.addEventListener('submit', function (e) {
  // PreventDefault
  e.preventDefault();

  // Get the value in numerical form
  const value = parseInt(amount.value); // Any input is always string, that is why parseInt is needed
  // Produce a random number
  const random = Math.floor(Math.random() * text.length);

  // Empty value (NaN), less than 0, more than 9
